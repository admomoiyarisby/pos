...
**Server-Function Core (Inti Fungsi Server)**:
The user-parameterized business logic behind a `createServerFn` endpoint (`createMutasiTransferCore`, `createProcurementCore`, `generateSCMInvoiceCore`, …). Each core is the handler body verbatim — role and branch guards, FSM transitions, stock effects, ledger writes, notifications — minus the HTTP session: it takes an explicit `AppUser` instead of calling `requireAuth()` / `requireRole()`. The endpoint wrapper keeps its validator and auth calls and delegates to the core. The core mirrors the wrapper's role guard so wrong-role actors are rejected when called directly. This seam exists so the full-flow integration tests (`*-flow.integration.test.ts`) can walk every state through the **real** logic with impersonated actors — `createServerFn` references cannot be invoked in-process (they require the Start request context). See ADR 0015.
_Avoid_: handler, transport function (when referring to the business logic — the handler is the HTTP wrapper, the core is the logic)
