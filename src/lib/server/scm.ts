...
export const createPurchaseRequisition = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireAuth();
    return createPurchaseRequisitionCore(user, data);
  });

/** The business logic behind `createPurchaseRequisition`, parameterized by an
 *  explicit user so it can be driven directly (e.g. from integration tests). */
export async function createPurchaseRequisitionCore(
  user: AppUser,
  data: {
    branchId: string;
    code: string;
...
export const updatePurchaseRequisition = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireAuth();
    return updatePurchaseRequisitionCore(user, data);
  });

export async function updatePurchaseRequisitionCore(
  user: AppUser,
  data: {
    id: string;
    items?: { ingredientId: string; quantity: number }[];
...
export const processPurchaseRequisition = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireAuth();
    return processPurchaseRequisitionCore(user, data);
  });

export async function processPurchaseRequisitionCore(
  user: AppUser,
  data: { id: string; alsoCreateSJ: boolean; driverName?: string; vehicleNumber?: string },
) {
...
export const createPurchaseOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return createPurchaseOrderCore(user, data);
  });

/** The business logic behind `createPurchaseOrder`, parameterized by an
 *  explicit user so it can be driven directly (e.g. from integration tests).
...
export const updatePurchaseOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return updatePurchaseOrderCore(user, data);
  });

export async function updatePurchaseOrderCore(
  user: AppUser,
  data: {
    id: string;
    items?: { ingredientId: string; quantity: number; unitPrice?: number }[];
...
export const sendPurchaseOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return sendPurchaseOrderCore(user, data);
  });

export async function sendPurchaseOrderCore(user: AppUser, data: { id: string }) {
  if (user.role !== "super_admin" && user.role !== "admin_pusat") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin | admin_pusat)`,
    );
...
export const receivePurchaseOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return receivePurchaseOrderCore(user, data);
  });

export async function receivePurchaseOrderCore(
  user: AppUser,
  data: { id: string; items: { itemId: string; receivedQuantity: number }[] },
) {
  if (user.role !== "super_admin" && user.role !== "admin_pusat") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin | admin_pusat)`,
...
export const cancelPurchaseOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return cancelPurchaseOrderCore(user, data);
  });

export async function cancelPurchaseOrderCore(user: AppUser, data: { id: string }) {
  if (user.role !== "super_admin" && user.role !== "admin_pusat") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin | admin_pusat)`,
    );
...
export const createDeliveryNote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return createDeliveryNoteCore(user, data);
  });

export async function createDeliveryNoteCore(
  user: AppUser,
  data: {
    code: string;
    prId?: string;
  ...
  await logSystemAction(user, "Create Delivery Note", `SJ "${data.code}" dibuat oleh ${user.name}`);
...
export const updateDeliveryNote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return updateDeliveryNoteCore(user, data);
  });

export async function updateDeliveryNoteCore(
  user: AppUser,
  data: {
    dnId: string;
    items: { itemId: string; pickedQuantity: number }[];
...
export const shipDeliveryNote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return shipDeliveryNoteCore(user, data);
  });

export async function shipDeliveryNoteCore(user: AppUser, data: { dnId: string }) {
  if (user.role !== "super_admin" && user.role !== "admin_pusat") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin | admin_pusat)`,
    );
  ...
  for (const item of items) {
    ...
    const [inv] = await db
      ...
      .from(inventory)
      .where(
        and(eq(inventory.branchId, dn.fromBranchId), eq(inventory.ingredientId, item.ingredientId)),
      )
      .limit(1);
...
export const receiveDeliveryNote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireAuth();
    return receiveDeliveryNoteCore(user, data);
  });

export async function receiveDeliveryNoteCore(
  user: AppUser,
  data: {
    dnId: string;
    items: {
    ...
    if (item.rejectedQuantity > 0) {
      if (item.rejectionDisposition === "Return to Source") {
        ...
        const [sourceInv] = await db
          ...
          .from(inventory)
          .where(
            and(eq(inventory.branchId, dn.fromBranchId), eq(inventory.ingredientId, ingredientId)),
          )
          .limit(1);
...
export const cancelDeliveryNote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return cancelDeliveryNoteCore(user, data);
  });

export async function cancelDeliveryNoteCore(
  user: AppUser,
  data: { dnId: string; reason: string },
) {
  if (user.role !== "super_admin" && user.role !== "admin_pusat") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin | admin_pusat)`,
...
export const reviewDeliveryNote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return reviewDeliveryNoteCore(user, data);
  });

export async function reviewDeliveryNoteCore(user: AppUser, data: { dnId: string }) {
  if (user.role !== "super_admin" && user.role !== "admin_pusat") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin | admin_pusat)`,
    );
  ...
  await logSystemAction(user, "Review Delivery Note", `SJ "${dn.code}" direview oleh ${user.name}`);
...
export const generateSCMInvoice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return generateSCMInvoiceCore(user, data);
  });

export async function generateSCMInvoiceCore(user: AppUser, data: { dnId: string }) {
  if (user.role !== "super_admin" && user.role !== "admin_pusat") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin | admin_pusat)`,
    );
...
export const paySCMInvoice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return paySCMInvoiceCore(user, data);
  });

export async function paySCMInvoiceCore(user: AppUser, data: { id: string }) {
  if (user.role !== "super_admin" && user.role !== "admin_pusat") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin | admin_pusat)`,
    );
  }
  const [oldInv] = await db.select().from(scmInvoices).where(eq(scmInvoices.id, data.id)).limit(1);
...
export const cancelSCMInvoice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return cancelSCMInvoiceCore(user, data);
  });

export async function cancelSCMInvoiceCore(user: AppUser, data: { id: string }) {
  if (user.role !== "super_admin" && user.role !== "admin_pusat") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin | admin_pusat)`,
    );
...
export const createStockTransfer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireAuth();
    return createStockTransferCore(user, data);
  });

export async function createStockTransferCore(
  user: AppUser,
  data: {
    code: string;
    fromBranchId: string;
...
export const approveStockTransfer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return approveStockTransferCore(user, data);
  });

export async function approveStockTransferCore(user: AppUser, data: { transferId: string }) {
  ...
  if (transfer.status !== "Pending Approval") throw new Error("Transfer must be Pending Approval");
...
export const rejectStockTransfer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireAuth();
    await requireRole("super_admin", "area_manager");
    return rejectStockTransferCore(user, data);
  });

export async function rejectStockTransferCore(
  user: AppUser,
...
export const shipStockTransfer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return shipStockTransferCore(user, data);
  });

export async function shipStockTransferCore(user: AppUser, data: { transferId: string }) {
  if (!["super_admin", "admin_pusat", "branch_admin"].includes(user.role)) {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin | admin_pusat | branch_admin)`,
    );
  }
...
export const receiveStockTransfer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return receiveStockTransferCore(user, data);
  });

export async function receiveStockTransferCore(user: AppUser, data: { transferId: string }) {
  if (user.role !== "super_admin" && user.role !== "branch_admin") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin | branch_admin)`,
    );
  }
...
export const cancelStockTransfer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin", "admin_pusat");
    return cancelStockTransferCore(user, data);
  });

export async function cancelStockTransferCore(
  user: AppUser,
  data: { transferId: string; reason: string },
) {
  if (user.role !== "super_admin" && user.role !== "admin_pusat") {
