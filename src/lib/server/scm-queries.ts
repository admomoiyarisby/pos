...
export const transitionProcurement = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Branch guard: the FSM authorizes by role; this closes the branch
    // dimension (mirrors Mutasi's assertTransferAccess + branchGuard).
    await assertProcurementAccess(user, data.procurementId);
...
export const updateProcurementItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Branch guard: branch_admin may only edit items of their own branch's
    // procurement (admin_pusat edits caDecision/readyQuantity during review).
    await assertProcurementAccess(user, data.procurementId);
