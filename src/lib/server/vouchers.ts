...
export async function createVoucherCore(user: AppUser, data: z.output<typeof voucherInput>) {
...
export const createVoucher = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return createVoucherCore(user, data);
  });
...
// User-parameterized core (ADR-0015). Mirrors the wrapper's requireRole guard.
export async function updateVoucherCore(user: AppUser, data: z.output<typeof updateVoucherInput>) {
  if (user.role !== "super_admin") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin)`,
    );
...
export const updateVoucher = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin");
    return updateVoucherCore(user, data);
  });

// User-parameterized core (ADR-0015). Mirrors the wrapper's requireRole guard.
export async function deactivateVoucherCore(user: AppUser, data: { id: string }) {
  if (user.role !== "super_admin") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin)`,
    );
...
export const deactivateVoucher = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin");
    return deactivateVoucherCore(user, data);
  });
...
// User-parameterized core (ADR-0015). Mirrors the wrapper's requireRole guard.
export async function deleteVoucherCore(user: AppUser, data: { id: string }) {
  if (user.role !== "super_admin") {
    throw new Error(
      `Forbidden: insufficient role (user ${user.id} has role "${user.role}", required: super_admin)`,
    );
...
export const deleteVoucher = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const user = await requireRole("super_admin");
    return deleteVoucherCore(user, data);
  });
