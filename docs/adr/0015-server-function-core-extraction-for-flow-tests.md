...
4. **Only mutation/lifecycle functions get cores — by default.** Read-only `GET` endpoints that just `requireAuth()` and query are normally left untouched. Exception: when a read-path contract needs pinning, extract the same way — `getStockLedgerCore` (`inventory.ts`) was extracted so the Kartu Stok pagination-determinism test can drive the ORDER BY / LIMIT-OFFSET contract with an explicit actor.
