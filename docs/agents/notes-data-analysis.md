...
gating, UI, and notification *logic* are out of scope here (tracked separately).
...
Proposed data change:
  (a) Add ingredient "Nasi Putih" (skuType FG or SFG) so it can appear in
      `stockOpnameItems.ingredientId`. Keep rec009 as the recipe that *consumes*
  ...
  (b) Add a reverse-conversion rule so a Nasi variance flows to its BOM
      ingredients. Options:
        - Reuse `yieldConversions` with a flag reverseEligible / or a new
          `nasiConversions` table mapping 1 Nasi → {Beras 72.9, Beras Ketan 8.1,
          Air 117, Cuka Nasi 1.25, Mangkok 1} (matches Audit Inventory rows 58–62).
        - On realize (note W), if Nasi variance != 0, write stockLedger OUT/IN
...
SHOULD (optional, improves grouping):
  5. operationalExpenses.category → controlled enum (notes E, H)
